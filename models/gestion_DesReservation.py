...
class Resevation(models.Model):
    ...
    reservation_ref = fields.Char(string='RESERVE', readonly=True, copy=False)

    client_id = fields.Many2one('res.users',ondelete='cascade', string="client", index=True)
    ...
    hours_computed = fields.Integer(string="hours number", store=True, compute='_get_hours')

    month_computed = fields.Integer(string="End Date", store=True, compute='_get_month')

    reservation_count = fields.Integer(string="Attendees count", compute='_get_reservation_count',
                                       store=True)
    ...
    def _get_hours(self):
        for r in self:
            r.hours_computed = (r.month * 30 + r.days) * 24 + r.hours
    ...
    def _get_month(self):
        ...
        for r in self:
            ...
            r.month_computed = start_date.month
    ...
    def create_quotation(self):
        # methode creer un devi associer a un reservation
        ...
        sale_order_cost = self.sale_order.create({
        ...
        self.sale_order = sale_order_cost
    ...
    def many_quotation(self):
        # methode creer plusieurs devis associer a des reservations par client groupement
        ...
        for user in users:
            ...
            for res in reserve:
                if res.state == 'valid':
                    ...
                    ER = True
            res.sale_order = sale_order_cost
        if ER:
